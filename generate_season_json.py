#!/usr/bin/env python3
"""
PropEdge V14.0 — Generate Season JSONs
========================================
...
def _write_summary(plays_2526: list[dict], plays_2425: list[dict]) -> None:
    ...
    def _s_season(plays):
        ...
        pl    = sum(
            p["units"] * 0.909 if p["result"] == "WIN" else -p["units"]
            for p in g if p["units"] > 0
        )
        monthly: dict = {}
        for p in t12:
            m = p["date"][:7]
...
def main():
    ...
    plays_2526 = [p for p in [] if True]  # re-read for reporting
    for season_label, season_filter in [("2025-26", "2025"), ("2024-25", "2024")]:
        ...
        t12_wins = (t12["result"] == "WIN").sum()
        t12_pct = f"{t12_wins/len(t12)*100:.1f}%" if len(t12) else "—"
        print(f"\n  {season_label}: {len(rows):,} plays  |  "
              f"All: {wins}W/{len(graded)-wins}L = {pct}  |  "
              f"T1+T2: {t12_wins}W/{len(t12)-t12_wins}L = {t12_pct}")

    print(f"\n  ✓ Generate complete  ({time.time()-t0:.0f}s)")
    print("=" * 60)
    log_event("GEN", "GENERATE_COMPLETE", detail=f"elapsed={time.time()-t0:.0f}s")
